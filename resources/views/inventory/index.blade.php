...
@section('content')
    <div>
        {{-- CATEGORY HEADER --}}
        <div class="space-x-1">
            <a href="{{ route('inventory.show') }}" class="px-5 py-2 bg-[#FFC107] rounded-t-xl">All items</a>

            @foreach ($categories as $category)
                <a href="{{ route('inventory.categorized', ['name' => $category->category_name]) }}"
                    class="px-5 py-2 bg-[#FFC107] rounded-t-xl"> {{ $category->category_name }} </a>
            @endforeach
        </div>
        ...
        <div class="bg-white rounded-3xl px-8 pb-8 flex flex-col items-center justify-center">
            ...
            <div class="flex items-center justify-between px-4 mt-7 w-full py-5 border-b border-gray-200">
                ...
                <button
                    class="flex items-center justify-center shadow-md hover:bg-[#005C39] transition text-white bg-[#00754A] px-4 py-2 rounded-lg text-sm"><span><img
                            class="w-5 mr-2" src="{{ asset('src/images/add.png') }}" alt="add icon"></span> Add item
                </button>
            </div>
            ...
            <div class="flex items-center justify-between px-4 w-full py-9 border-b border-gray-200">
                <div class="flex w-72 border border-gray-200 p-2 rounded-lg ">
                    <img class="w-[24px]" src="{{ asset('src/images/search.png') }}" alt="search icon">
                ...
                <div class="flex w-70 py-2 px-3 rounded-3xl bg-[#E5E5E5] text-gray-500">
                    <img class="w-[24px]" src="{{ asset('src/images/filter.png') }}" alt="search icon">
                    <select name="filter" id="filter" class="ml-2 outline-none bg-transparent w-full focus:border-transparent">
